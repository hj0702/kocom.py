...
def init_mqttc():
    ...
    # ✅ 세대현관 통화 후 문열림 자동화
    elif msg.topic == 'kocom/door/open':
        try:
            payload = json.loads(msg.payload.decode())
            if payload.get("action") == "call_and_open":
                logging.info("[AUTO] 세대현관 통화 + 문열림 자동화 실행")

                # 1. 세대현관 통화 패킷
                call_packets = [
                    "aa5530bc0033000100010000000000000000210d0d",
                    "aa5530bd0033000100010000000000000000220d0d",
                    "aa5530be0033000100010000000000000000230d0d"
                ]
                for packet in call_packets:
                    rs485.write(bytearray.fromhex(packet))
                    time.sleep(0.3)

                logging.info("[AUTO] 세대현관 통화 완료")

                time.sleep(1.2)  # 통화 후 딜레이

                # 2. 문열림 패킷
                open_packets = [
                    "aa5530bc00600001000200000000000000004f0d0d",
                    "aa5530bd0060000100020000000000000000500d0d",
                    "aa5530be0060000100020000000000000000510d0d"
                ]
                for packet in open_packets:
                    rs485.write(bytearray.fromhex(packet))
                    time.sleep(0.3)

                logging.info("[AUTO] 세대현관 문열림 완료")
        except Exception as e:
            logging.error(f"[AUTO] call_and_open 처리 오류: {e}")
